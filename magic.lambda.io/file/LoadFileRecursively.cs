...
namespace magic.lambda.io.file
{
    ...
    public class LoadFileRecursively : ISlot, ISlotAsync
    {
        ...
        {
            input.Clear();
            foreach (var idx in _service.LoadRecursively(_rootResolver.AbsolutePath(input.GetEx<string>()), null))
            {
        ...
        {
            input.Clear();
            foreach (var idx in await _service.LoadRecursivelyAsync(_rootResolver.AbsolutePath(input.GetEx<string>()), null))
            {
                var idxNode = new Node(".");
                idxNode.Add(new Node("name", _rootResolver.RelativePath(idx.Filename)));
                idxNode.Add(new Node("content", Encoding.UTF8.GetString(idx.Content)));
